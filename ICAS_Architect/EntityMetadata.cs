...
namespace ICAS_Architect
{
    ...
    internal class EntityMetadata
    {
        ...
        internal EntityMetadata(string organizationUrl, string APIURL, string metadataId, string entityLogicalName, string entitySetName, string descriptiontext, int objectTypeCode, string primaryIdAttribute, string primaryNameAttribute, string displayNamesText, bool isIntersect)
        {
            ...
            erInformationForThisEntity.EREntities.Add(new EREntity(organizationUrl, APIURL, metadataId, entityLogicalName, entitySetName, descriptiontext, objectTypeCode, primaryIdAttribute, primaryNameAttribute, displayNamesText, isIntersect, "API", "","" ));
        }
        ...
        private void DumpIntersectInformation(string entityLogicalName, string hashCodeHeader, JToken rootAttributes, StreamWriter specWriter)
        {
            ...
            if (Entity1LogicalName.CompareTo("systemuser") == 0)
            {
                //gg: Skip any FK where Entity1 = systemuser
            }
            else
            {
...
//            erInformationForThisEntity.EREntitieAttributes.Add(new EREntityAttribute(entityLogicalName, string.Empty, "Entity", $"({entitySetName}) " + completeDesc, 0, "", false, "", ""));
        ...
        private void DumpEntityAttributes(string entityLogicalName, string hashCodeHeader, JToken rootAttributes, StreamWriter specWriter)
        {
            ...
            { //gg: Add MetadataID, ColumnNumber, IsPrimaryID, AttributeType, DisplayName
                int columnNumber = 0;
                ...
                if (attribute["ColumnNumber"] != null) columnNumber = attribute["ColumnNumber"].Value<int>();
                if (attribute["EntityLogicalName"] != null) entityLogicalName2 = attribute["EntityLogicalName"].Value<string>();
                if (attribute["IsPrimaryID"] != null) isPrimaryID = attribute["IsPrimaryID"].Value<bool>();
                if (attribute["AttributeType"] != null) attributeType = attribute["AttributeType"].Value<string>();
                if (attribute["MetadataId"] != null) metadataId = attribute["MetadataId"].Value<string>();
...
//                erInformationForThisEntity.EREntitieAttributes.Add(new EREntityAttribute(entityLogicalName, schemaName, dataType, description));
                erInformationForThisEntity.EREntitieAttributes.Add(new EREntityAttribute(entityLogicalName, schemaName, dataType, description, columnNumber, entityLogicalName2, isPrimaryID, attributeType, metadataId));
            }
        }
